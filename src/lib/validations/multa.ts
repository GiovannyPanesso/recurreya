...
export const multaSchema = z
  .object({
    ...
    acepto_no_abogados: z.literal(true, {
      message: "Debes aceptar este aviso",
    }),
  })
  .refine(
    (data) => {
      if (data.organismo_emisor === "ayuntamiento") {
        return (
          !!data.municipio_emisor && data.municipio_emisor.trim().length > 0
        );
      }
      return true;
    },
    {
      message: "Indica el municipio del Ayuntamiento que emitió la multa",
      path: ["municipio_emisor"],
    },
  );
